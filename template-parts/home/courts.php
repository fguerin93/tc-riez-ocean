...
<section id="courts" class="bg-cream py-20 md:py-28">
	<div class="max-w-7xl mx-auto px-5 sm:px-8 md:px-16">
		...
		<div class="mb-10 reveal">
			...
			<?php endif; ?>
			<h2 class="font-display font-bold text-ink leading-tight" style="font-size:clamp(26px,4vw,48px)">
		...
		<div class="grid grid-cols-1 sm:grid-cols-3 gap-0.5 reveal">
			...
			<div class="group relative overflow-hidden cursor-pointer" style="aspect-ratio:3/2">
				...
				<div class="absolute inset-0 flex flex-col justify-end p-5 md:p-7" style="background:linear-gradient(to top,rgba(10,21,36,.85),transparent 55%)">
					<?php if ( ! empty( $item['sous_titre'] ) ) : ?><p class="text-white/70 text-[10px] tracking-[.2em] uppercase mb-1"><?php echo esc_html( $item['sous_titre'] ); ?></p><?php endif; ?>
					<h3 class="font-display font-bold text-white text-xl md:text-2xl mb-1.5"><?php echo esc_html( $item['nom'] ?? '' ); ?></h3>
					<?php if ( ! empty( $item['description'] ) ) : ?><p class="text-white/60 text-sm leading-relaxed hidden sm:block"><?php echo esc_html( $item['description'] ); ?></p><?php endif; ?>
				</div>
			</div>
			<?php endforeach; ?>
		</div>
		...
		<div class="grid grid-cols-1 md:grid-cols-3 gap-0.5 mt-0.5 reveal">
			<div class="md:col-span-2 group relative overflow-hidden cursor-pointer min-h-48 md:min-h-64">
				...
				<div class="absolute inset-3 md:inset-5 border border-white/10 pointer-events-none"></div>
				<div class="absolute inset-0 flex flex-col justify-end p-5 md:p-10" style="background:linear-gradient(to top,rgba(10,21,36,.8),transparent 50%)">
					...
					<h3 class="font-display font-bold text-white text-xl md:text-3xl mb-1"><?php echo esc_html( $feat_nom ); ?></h3>
					<?php if ( $feat_desc ) : ?><p class="text-white/60 text-sm hidden sm:block"><?php echo esc_html( $feat_desc ); ?></p><?php endif; ?>
				</div>
			</div>

			<?php if ( ! empty( $recap ) ) : ?>
			<div class="bg-ink/[.04] border-t md:border-t-0 md:border-l border-ink/[.07] p-5 md:p-8 grid grid-cols-2 md:grid-cols-1 gap-4 md:gap-6">
				<?php foreach ( $recap as $r ) : ?>
					<div>
						<p class="text-ink/35 text-[10px] tracking-widest uppercase mb-1"><?php echo esc_html( $r['label'] ); ?></p>
						<p class="font-display font-semibold text-ink"><?php echo esc_html( $r['valeur'] ); ?></p>
						<?php if ( ! empty( $r['detail'] ) ) : ?><p class="text-ink/45 text-xs"><?php echo wp_kses( $r['detail'], [ 'a' => [ 'href' => [], 'target' => [], 'class' => [] ] ] ); ?></p><?php endif; ?>
					</div>
				<?php endforeach; ?>
			</div>
			<?php endif; ?>
